"""Run project with ComboExtractor (Rule-Based V2 + LLM Refiner)."""
import argparse
import json
import sys
...
from src.extraction.combo_extractor import ComboExtractor
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"AI Takeoff Builder COMBO - {args.project_id}")
    print(f"{'='*60}\n")
    ...
    # Step 2: Extract with Combo
    print("Step 2: Extracting line items with ComboExtractor...")
    engine = ComboExtractor()
    ...
    ai_run = AIRun(
        run_id="combo-v1",
        tools_or_models_used=["rule-based-extractor-v2", "llm-refiner", "PyMuPDF", "pdfplumber"],
        assumptions=["Combo approach: fast rule-based extraction + targeted LLM refinement on critical sections (schedules, legends)"],
        warnings=["Quantities may be null when not explicitly stated in text"]
    )
